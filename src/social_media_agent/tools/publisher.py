...
from social_media_agent.utils.mcp_client import XiaohongshuMCPClient, XiaohongshuMCPError
    ...
    try:
        ...
        try:
            ...
            return json.dumps({
                ...
                "url": result.get("url", result.get("note_url", "")),
                "message": "发布成功",
                "data": result
            }, ensure_ascii=False)
...
def publish_to_xiaohongshu_native(
    title: str,
    content: str,
    images: Optional[List[str]] = None,
    video_path: Optional[str] = None,
    ...
    """Structured dict interface for publisher tool."""
    raw = publish_to_xiaohongshu(
        title=title,
        content=content,
        images=images,
        video_path=video_path,
    ...
    try:
        ...
        if isinstance(parsed, dict):
            return parsed
        return {"success": True, "data": parsed, "message": "发布完成"}
    except Exception:
        return {"success": False, "error": str(raw), "message": "发布失败"}
