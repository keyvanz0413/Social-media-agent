...
def _requests_available() -> bool:
    """检查 requests 依赖是否可用。"""
    if requests is None:
        logger.error("requests 未安装，当前方法不可用")
        return False
    return True
...
def generate_images_for_content_native(
    image_suggestions: str,
    topic: str,
    count: Optional[Union[int, str]] = None,
    method: str = "dalle",
    ...
    """Structured dict interface for image generation."""
    raw = generate_images_for_content(
    ...
    try:
        ...
        if isinstance(parsed, dict):
            return parsed
        return {"success": True, "data": parsed, "message": "图片生成完成"}
    except Exception:
        return {"success": False, "error": str(raw), "message": "图片生成失败"}
...
def generate_images_from_draft_native(
    draft_id: str,
    method: str = "dalle",
    count: Optional[int] = None
) -> Dict[str, Any]:
    """Structured dict interface for draft image generation."""
    raw = generate_images_from_draft(draft_id=draft_id, method=method, count=count)
    if isinstance(raw, dict):
        return raw
    try:
        parsed = json.loads(raw)
