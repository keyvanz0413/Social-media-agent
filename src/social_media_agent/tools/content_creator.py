...
def create_content_native(
    analysis_result: Any,
    topic: str,
    style: str = "casual",
    quality_level: str = "balanced"
) -> Dict[str, Any]:
    """Create content and return structured dict result."""
    try:
        # 1. 解析分析结果
    ...
    if analysis_data:
        ...
        if "creation_suggestions" in analysis_data:
            ...
            if isinstance(suggestions, list) and suggestions:
                ...
                # 兼容旧格式：creation_suggestions 可能是结构化字典
                angle = suggestions.get("title_style") or suggestions.get("structure") or ""
                if angle:
                    prompt_parts.append(f"- 推荐角度: {angle}")
