...
def review_compliance(content_data: dict, quality_level: str = "balanced") -> str:
    ...
    """
    return json.dumps(
        review_compliance_native(content_data=content_data, quality_level=quality_level),
        ensure_ascii=False,
    )


def review_compliance_native(content_data: dict, quality_level: str = "balanced") -> dict:
    """评审合规性，返回结构化 dict。"""
    try:
