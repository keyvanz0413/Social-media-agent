...
#### `create_schedule(topic: str, days: int = 7, frequency: str = "daily", start_date: str = None, preferred_time: str = "20:00", platform: str = "xiaohongshu")`
创建内容排期（日历）。

**适用场景**：
- 用户要求“未来一周排期”“做内容日历”“规划连更计划”
- 批量生成选题发布时间

#### `list_schedule(date_from: str = None, date_to: str = None, status: str = None, limit: int = 50)`
查询排期列表，支持按日期范围和状态过滤。

#### `reschedule(item_id: int, new_time: str)`
调整指定排期项的发布时间。

**排期决策规则**：
- 当用户明确要求“排期/日历/计划发布”，优先调用排期工具而不是直接发布
- 先 `create_schedule` 再 `list_schedule` 汇总给用户确认
- 用户指定修改某一条时间时使用 `reschedule`

---

### 5. 长期记忆工具

#### `save_memory(item_type: str, content: str, metadata: str = None, source: str = "agent")`
保存关键任务信息到长期记忆库（FAISS 优先，缺依赖时自动降级）。
...
#### `search_memory(query: str, top_k: int = 5, item_type: str = None)`
在长期记忆中检索与当前任务相关的历史经验。

#### `list_recent_memories(limit: int = 20, item_type: str = None)`
查看最近的记忆写入，便于复盘。

**记忆使用规则**：
- 当任务涉及“延续历史风格/复用过往经验”时，先检索记忆再创作
- 任务结束后，保存本次关键结论（尤其是评审建议与用户偏好）
