...
class XiaohongshuMCPClient:
    ...
    def __init__(
        ...
        if requests is None:
            if DevConfig.MOCK_MODE:
                self.session = None
                logger.warning("requests 未安装，MCP 客户端仅支持 Mock 模式")
                return
            raise XiaohongshuMCPError("requests 未安装，无法调用 MCP HTTP 接口")
    ...
    def _make_request(
        ...
        if requests is None or self.session is None:
            raise XiaohongshuMCPError("requests 未安装，无法发起 HTTP 请求")
