...
def create_error_response(error: str, message: str = "操作失败") -> str:
    """向后兼容：创建标准错误响应。"""
    from social_media_agent.utils.response_utils import create_error_response as _create_error_response
    return _create_error_response(error=error, message=message)


def create_success_response(data: Any = None, message: str = "操作成功") -> str:
    """向后兼容：创建标准成功响应。"""
    from social_media_agent.utils.response_utils import create_success_response as _create_success_response
    return _create_success_response(data=data, message=message)
